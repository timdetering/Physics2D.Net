...
namespace Physics2D.Joints
{
	...
	/// </summary>
    [Serializable]
    public class TrackedAnchor :BaseRemovableObject, IJoint
	{
		ICollidableBody body;
		Vector2D pointRelativeToICollidableBody1;
		Vector2D pointRelativeToWorld;
		Vector2D trackTangent;
		float distanceAllowed;
		float timescale;
        JointBreakInfo breakInfo;
		public TrackedAnchor(ICollidableBody body,Vector2D pointRelativeToWorld,Vector2D trackTangent,float distanceAllowed,float timescale)
            : this(new Immortal(), body, pointRelativeToWorld, trackTangent, distanceAllowed, timescale, JointBreakInfo.Unbreakable)
		
        {}
        public TrackedAnchor(ICollidableBody body, Vector2D pointRelativeToWorld, Vector2D trackTangent, float distanceAllowed, float timescale, float breakDistance, float breakVelocity, float breakImpulse)
            : this(new Immortal(),body,pointRelativeToWorld,trackTangent,distanceAllowed,timescale,new JointBreakInfo(breakDistance,breakVelocity,breakImpulse))
		{}
        public TrackedAnchor(ILifeTime lifeTime, ICollidableBody body, Vector2D pointRelativeToWorld, Vector2D trackTangent, float distanceAllowed, float timescale, JointBreakInfo breakInfo)
            : base(lifeTime)
        {
            this.body = body;
            this.pointRelativeToWorld = pointRelativeToWorld;
            this.trackTangent = trackTangent;
		...
		public void PreCalc(float dt)
		{
			...
			float deviationMag = deviation*trackTangent;
			float tmpTimeScale;
			if(timescale > 0)
			{
			...
			if (deviationMag > distanceAllowed)
			{
				...
				extraVelocity = ((deviationMag - distanceAllowed) / tmpTimeScale) * trackTangent;
			}
			else
			{
		...
		public bool CalcAndApply(float dt)
		{
			...
			//	return false;
			//}
			float relativeVelocityMag = relativeVelocity*trackTangent;
			//Vector2D relativeVelocityNormal = relativeVelocityMag * trackTangent;//(1/relativeVelocityMag);
			...
			float K = body.GetK( rPoint1, trackTangent); // + collidables.Collidable2.GetK( rPoint2, relativeVelocityNormal);
			...
			if(!body.CollisionState.Frozen)
			{
				body.ApplyImpulse(rPoint1,(impulse)*trackTangent);
			}
